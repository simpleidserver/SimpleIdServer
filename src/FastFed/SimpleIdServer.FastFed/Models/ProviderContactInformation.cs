...
public class ProviderContactInformation
{
    ...
    public List<string> Validate()
    {
        ...
        if (string.IsNullOrWhiteSpace(Organization)) result.Add(string.Empty);
        if (string.IsNullOrWhiteSpace(Phone)) result.Add(string.Empty);
        if (string.IsNullOrWhiteSpace(Email)) result.Add(string.Empty);
        return result;
    }
}
