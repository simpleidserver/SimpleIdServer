...
namespace SimpleIdServer.OpenID.Api.Token.TokenBuilders
{
    public class IdTokenBuilder : ITokenBuilder
    {
        ...
        public async Task Build(IEnumerable<string> scopes, Dictionary<string, object> claims, HandlerContext context, CancellationToken cancellationToken)
        {
            ...
            var idToken = await _jwtBuilder.BuildClientToken(context.Client, payload, openidClient.GetIdTokenSignedResponseAlg(), openidClient.GetIdTokenEncryptedResponseAlg(), openidClient.GetIdTokenEncryptedResponseEnc(), cancellationToken);
        ...
        public async Task Refresh(JsonObject previousQueryParameters, HandlerContext handlerContext, CancellationToken token)
        {
            ...
            var idToken = await _jwtBuilder.BuildClientToken(handlerContext.Client, payload, openidClient.GetIdTokenSignedResponseAlg(), openidClient.GetIdTokenEncryptedResponseAlg(), openidClient.GetIdTokenEncryptedResponseEnc(), token);
