...
namespace SimpleIdServer.OpenID.Helpers
{
    ...
    public class ExtractRequestHelper: IExtractRequestHelper
    {
        ...
        protected virtual async Task<bool> CheckRequest(HandlerContext context, string request)
        {
            ...
            var json = res.Jwt.GetClaimJson();
            context.Request.SetRequestData(json);
        ...
        protected virtual void CheckRequestObject(JsonWebToken jwt, Client client, HandlerContext context)
        {
            ...
            if (!string.IsNullOrWhiteSpace(client.GetRequestObjectSigningAlg()) && jwt.Alg != client.GetRequestObjectSigningAlg())
                throw new OAuthException(ErrorCodes.INVALID_REQUEST_OBJECT, ErrorMessages.INVALID_SIGNATURE_ALG);

            if (!jwt.TryGetClaim(OAuth.DTOs.AuthorizationRequestParameters.ResponseType, out Claim c))
                throw new OAuthException(ErrorCodes.INVALID_REQUEST_OBJECT, ErrorMessages.MISSING_RESPONSE_TYPE_CLAIM);

            if (!jwt.TryGetClaim(OAuth.DTOs.AuthorizationRequestParameters.ClientId, out Claim cl))
                throw new OAuthException(ErrorCodes.INVALID_REQUEST_OBJECT, ErrorMessages.MISSING_CLIENT_ID_CLAIM);

            if (!jwt.GetClaim(OAuth.DTOs.AuthorizationRequestParameters.ResponseType).Value.Split(' ').OrderBy(s => s).SequenceEqual(context.Request.RequestData.GetResponseTypesFromAuthorizationRequest().OrderBy(s => s)))
                throw new OAuthException(ErrorCodes.INVALID_REQUEST_OBJECT, ErrorMessages.INVALID_RESPONSE_TYPE_CLAIM);

            if (jwt.GetClaim(OAuth.DTOs.AuthorizationRequestParameters.ClientId).Value != context.Client.ClientId)
