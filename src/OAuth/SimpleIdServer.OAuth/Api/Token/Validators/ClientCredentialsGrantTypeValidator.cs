...
namespace SimpleIdServer.OAuth.Api.Token.Validators
{
    ...
    public class ClientCredentialsGrantTypeValidator : IClientCredentialsGrantTypeValidator
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(context.Request.RequestData.GetStr(TokenRequestParameters.Scope))) throw new OAuthException(ErrorCodes.INVALID_REQUEST, string.Format(ErrorMessages.MISSING_PARAMETER, "scope"));
        }
    }
}