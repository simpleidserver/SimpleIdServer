...
{
    public class RSA15CEKHandler : ICEKHandler
    {
        public string AlgName => ALG_NAME;
        public const string ALG_NAME = "RSA1_5";
        ...
        public byte[] Encrypt(byte[] payload, JsonWebKey jsonWebKey)
        {
            using (var rsa = RSA.Create())
            {
                ...
                return rsa.Encrypt(payload, RSAEncryptionPadding.Pkcs1);
            }
        }
        ...
        public byte[] Decrypt(byte[] payload, JsonWebKey jsonWebKey)
        {
            ...
            {
                rsa.Import(jsonWebKey.Content);
                return rsa.Decrypt(payload, RSAEncryptionPadding.Pkcs1);
            }
        }
    }
}