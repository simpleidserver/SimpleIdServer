...
{
    public abstract class RSASignHandler : ISignHandler
    {
        public string Sign(string payload, JsonWebKey jsonWebKey)
        {
            ...
            using (var rsa = RSA.Create())
            {
                ...
                return rsa.SignData(bytesToBeSigned, Alg, RSASignaturePadding.Pkcs1).Base64EncodeBytes();
            }
        }
        ...
        public bool Verify(string payload, byte[] signature, JsonWebKey jsonWebKey)
        {
            ...
            {
                rsa.Import(jsonWebKey.Content);
                return rsa.VerifyData(plainBytes, signature, Alg, RSASignaturePadding.Pkcs1);
            }
        }
