...
{
    public class CustomWsFederationHandler : RemoteAuthenticationHandler<WsFederationOptions>, IAuthenticationSignOutHandler
    {
        ...
        public CustomWsFederationHandler(Microsoft.Extensions.Options.IOptionsMonitor<WsFederationOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock)
...
#pragma warning restore CS8620 // Argument cannot be used for parameter due to differences in the nullability of reference types.
            ...
            try
            {
                ...
                {
                    return HandleRequestResult.Fail("SignInMessageWresultIsMissing", properties);
                }

                var token = wsFederationMessage.GetToken();
                if (string.IsNullOrEmpty(token))
                {
                    return HandleRequestResult.Fail("SignInMessageTokenIsMissing", properties);
                }
                ...
                if (principal == null)
                {
                    throw new SecurityTokenException("Exception_NoTokenValidatorFound");
                }
