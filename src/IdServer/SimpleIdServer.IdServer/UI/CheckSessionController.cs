...
namespace SimpleIdServer.IdServer.UI
{
    public class CheckSessionController : BaseController
    {
        ...
        protected async Task<string> GetSessionId(string realm, CancellationToken cancellationToken)
        {
            ...
            var user = await _userRepository.Query().Include(u => u.Sessions).Include(u => u.Realms).FirstOrDefaultAsync(u => u.Name == userId && u.Realms.Any(r => r.RealmsName == realm), cancellationToken);
