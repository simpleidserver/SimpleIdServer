        ...
        private async Task<IActionResult> Common(string prefix, JsonObject content, CancellationToken cancellationToken)
        {
            ...
            using (var activity = Tracing.IdServerActivitySource.StartActivity("Get UserInfo"))
            {
                try
                {
                    ...
                    user = await _userRepository.Get(u => u.Include(u => u.Consents).ThenInclude(c => c.Scopes).ThenInclude(c => c.AuthorizedResources).Include(u => u.Groups).Include(u => u.OAuthUserClaims).AsNoTracking().FirstOrDefaultAsync(u => u.Name == subject, cancellationToken));
