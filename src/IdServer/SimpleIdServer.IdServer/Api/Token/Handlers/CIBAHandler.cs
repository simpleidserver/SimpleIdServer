        ...
        public override async Task<IActionResult> Handle(HandlerContext context, CancellationToken cancellationToken)
        {
            ...
            using (var activity = Tracing.IdServerActivitySource.StartActivity("Get Token"))
            {
                try
                {
                    ...
                    var user = await _userRepository.Query().Include(u => u.Groups).Include(u => u.OAuthUserClaims).Include(u => u.Realms).FirstOrDefaultAsync(u => u.Id == authRequest.UserId && u.Realms.Any(r => r.RealmsName == context.Realm), cancellationToken);
