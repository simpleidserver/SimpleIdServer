...
namespace SimpleIdServer.IdServer.Api.BCAuthorize
{
    ...
    public class BCAuthorizeRequestValidator : IBCAuthorizeRequestValidator
    {
        ...
        protected virtual async Task<User> CheckLoginHint(HandlerContext context, CancellationToken cancellationToken)
        {
            ...
            {
                var user = await _userRepository.Get(u => u.Include(u => u.Groups).Include(u => u.OAuthUserClaims).Include(u => u.Credentials).Include(u => u.Realms).Include(u => u.Devices).AsNoTracking().FirstOrDefaultAsync(u => u.Name == loginHint && u.Realms.Any(r => r.RealmsName == context.Realm), cancellationToken));
                if (user == null)
        ...
        protected virtual async Task<User> CheckHint(string realm, JsonWebToken jwsPayload, CancellationToken cancellationToken)
        {
            ...
            var user = await _userRepository.Get(u => u.Include(u => u.Groups).Include(u => u.OAuthUserClaims).Include(u => u.Credentials).Include(u => u.Realms).Include(u => u.Devices).AsNoTracking().FirstOrDefaultAsync(u => u.Name == subject && u.Realms.Any(r => r.RealmsName == realm), cancellationToken));
            if (user == null)
