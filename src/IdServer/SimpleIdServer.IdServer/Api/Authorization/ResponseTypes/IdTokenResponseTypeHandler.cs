...
namespace SimpleIdServer.IdServer.Api.Authorization.ResponseTypes
{
    public class IdTokenResponseTypeHandler : IResponseTypeHandler
    {
        ...
        {
            return _tokenBuilders.First(t => t.Name == AuthorizationResponseParameters.IdToken).Build(new BuildTokenParameter { Scopes = parameter.Scopes, AuthorizationDetails = parameter.AuthorizationDetails?.ToList(), Audiences = parameter.Audiences, Claims = parameter.Claims, GrantId = parameter.GrantId }, context, cancellationToken);
        }
    }
}
