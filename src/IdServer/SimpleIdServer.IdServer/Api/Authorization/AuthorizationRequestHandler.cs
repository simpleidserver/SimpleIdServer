    ...
    public class AuthorizationRequestHandler : IAuthorizationRequestHandler
    {
        ...
        public virtual async Task<AuthorizationResponse> Handle(HandlerContext context, CancellationToken token)
        {
            ...
            catch(OAuthAuthenticatedUserAmrMissingException ex)
            {
                ...
                return new RedirectActionAuthorizationResponse("Index", "Authenticate", context.Request.OriginalRequestData, ex.Amr, false, new List<string> { _options.GetSessionCookieName(context.Realm, context.Request.UserSubject), Constants.DefaultCurrentAmrCookieName }, amrAuthInfo);
            }
        }
