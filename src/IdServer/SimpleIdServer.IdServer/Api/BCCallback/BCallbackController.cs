...
namespace SimpleIdServer.IdServer.Api.BCAuthorize
{
    public class BCCallbackController : BaseController
    {
        ...
        public BCCallbackController(IJwtBuilder jwtBuilder, IBCAuthorizeRepository bCAuthorizeRepository)
        {
        ...
        public async Task<IActionResult> Post([FromBody] BCCallbackParameter parameter, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var bcAuthorize = await _bcAuthorizeRepository.Query().FirstOrDefaultAsync(b => b.Id == parameter.AuthReqId, cancellationToken);
                ...
                switch(parameter.Action)
                {
                    case BCCallbackActions.CONFIRM:
                        {
                            if (bcAuthorize.Status != Domains.BCAuthorizeStatus.Pending) return BuildError(HttpStatusCode.BadRequest, ErrorCodes.INVALID_REQUEST, ErrorMessages.BC_AUTHORIZE_NOT_PENDING);
                            bcAuthorize.Confirm();
                        }
                        break;
