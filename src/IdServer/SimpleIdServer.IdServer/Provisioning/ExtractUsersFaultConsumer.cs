...
namespace SimpleIdServer.IdServer.Provisioning
{
    public class ExtractUsersFaultConsumer : IConsumer<Fault<ExtractUsersCommand>>
    {
        ...
        public ExtractUsersFaultConsumer(ITransactionBuilder transactionBuilder, IMessageBusErrorStore messageBusErrorStore)
        {
            _transactionBuilder = transactionBuilder;
            _messageBusErrorStore = messageBusErrorStore;
        }

        public async Task Consume(ConsumeContext<Fault<ExtractUsersCommand>> context)
        {
            using (var transaction = _transactionBuilder.Build())
            {
                var type = typeof(ExtractUsersCommand);
                ...
                var message = new MessageBusErrorMessage
                {
                    ...
                    QueueName = ExtractUsersConsumer.Queuename
                };
