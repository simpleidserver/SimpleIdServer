...
public class UpdateParLitetimeDataSeeder : BaseAfterDeploymentDataSeeder
{
    private readonly ITransactionBuilder _transactionBuilder;
    private readonly IClientRepository _clientRepository;

    public UpdateParLitetimeDataSeeder(
    ...
    public override string Name => nameof(UpdateParLitetimeDataSeeder);
    ...
    protected override async Task Execute(CancellationToken cancellationToken)
    {
        using (var transaction = _transactionBuilder.Build())
        {
            ...
            {
                if (cl.PARExpirationTimeInSeconds == default)
                {
                    cl.PARExpirationTimeInSeconds = 600;
