...
namespace SimpleIdServer.Scim.SwashbuckleV6
{
    ...
    public class ScimApiDescriptionGroupCollectionProvider : IApiDescriptionGroupCollectionProvider
    {
        private const string _defaultVersion = "v1";
        private readonly IScimEndpointStore _edpsStore;
        ...
        public ScimApiDescriptionGroupCollectionProvider(
            IScimEndpointStore edpsStore,
        ...
        private ApiDescription CreateApiDescription(
            ...
            string? httpMethod,
            string? groupName,
            ScimConventionalRouteEntry route)
        {
            ...
            var parameterContext = new ScimApiParameterContext(_modelMetadataProvider, action, templateParameters);
            foreach (var parameter in GetParameters(parameterContext))
            {
                if (parameter.Name == "prefix") continue;
        ...
        private IList<ApiParameterDescription> GetParameters(ScimApiParameterContext context)
        {
        ...
        private void ProcessRouteParameters(ScimApiParameterContext context)
        {
        ...
        private static void ProcessParameterDefaultValue(ScimApiParameterContext context)
        {
        ...
        private static void ProcessIsRequired(ScimApiParameterContext context, MvcOptions mvcOptions)
        {
            foreach (var parameter in context.Results)
            {
                if (parameter.Source == BindingSource.Body)
        ...
        private static RouteTemplate? ParseTemplate(ScimConventionalRouteEntry route)
        {
        ...
        {
            public PseudoModelBindingVisitor(ScimApiParameterContext context, ParameterDescriptor parameter)
            {
                Context = context;
                Parameter = parameter;
            ...
            public ScimApiParameterContext Context { get; }
