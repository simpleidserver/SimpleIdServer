...
function install_sidwebsite_systemd {
    echo ""
    echo -e "${C_YELLOW}Install sidwebsite.service"
    echo -e "=======================${NC}"
    echo ""
    RUNDIR="$1"
    ...
    read -r -d '' IDSERVERWEBSITE_SYSTEMD_SERVICE_CONF << END
[Unit]
Description=Identity Server Website
...
TimeoutStopSec=300
User=USER
ExecStart=IDSERVERWEBSITE_DIR/run.sh

[Install]
WantedBy=multi-user.target
END

    export IDSERVERWEBSITE_SYSTEMD_SERVICE_CONF
    export SYSTEMD_SERVICE_FILE=/etc/systemd/system/sidwebsite.service
    sudo -E bash -c 'echo "$IDSERVERWEBSITE_SYSTEMD_SERVICE_CONF" > "$SYSTEMD_SERVICE_FILE"'
    sudo sed -i "s/USER/$EXEC_USER/g" "$SYSTEMD_SERVICE_FILE"
    sudo sed -i "s/IDSERVERWEBSITE_DIR/$ESC_RUNDIR/g" "$SYSTEMD_SERVICE_FILE"

    sudo systemctl daemon-reload
    sudo systemctl enable sidwebsite.service
    echo " "
    echo "Starting service..."
    sudo systemctl restart sidwebsite.service
    echo " "
    sleep 5
    sudo systemctl status sidwebsite.service
    echo " "
    echo "For details try journalctl -u sidwebsite.service"
    echo ""

}

echo -e "${C_L_GREEN}${T_BOLD}Install Identity Server Website /Service${NC}${NT}"
echo -e "${C_D_GRAY}==============================${NC}"
echo ""
...
if [ ${IS_UPSTART} -eq 1 ]; then
    ...
    install_sidwebsite_systemd "${DEFPATH}"
fi
