...
function install_sid_systemd {
    echo ""
    echo -e "${C_YELLOW}Install sid.service"
    echo -e "=======================${NC}"
    echo ""
    RUNDIR="$1"
    ...
    read -r -d '' IDSERVER_SYSTEMD_SERVICE_CONF << END
[Unit]
...
TimeoutStopSec=300
User=USER
ExecStart=IDSERVER_DIR/run.sh

[Install]
WantedBy=multi-user.target
END

    export IDSERVER_SYSTEMD_SERVICE_CONF
    export SYSTEMD_SERVICE_FILE=/etc/systemd/system/sid.service
    sudo -E bash -c 'echo "$IDSERVER_SYSTEMD_SERVICE_CONF" > "$SYSTEMD_SERVICE_FILE"'
    sudo sed -i "s/USER/$EXEC_USER/g" "$SYSTEMD_SERVICE_FILE"
    sudo sed -i "s/IDSERVER_DIR/$ESC_RUNDIR/g" "$SYSTEMD_SERVICE_FILE"

    sudo systemctl daemon-reload
    sudo systemctl enable sid.service
    echo " "
    echo "Starting service..."
    sudo systemctl restart sid.service
    echo " "
    sleep 5
    sudo systemctl status sid.service
    echo " "
    echo "For details try journalctl -u sid.service"
    echo ""

}

echo -e "${C_L_GREEN}${T_BOLD}Install Identity Server /Service${NC}${NT}"
echo -e "${C_D_GRAY}==============================${NC}"
echo ""
