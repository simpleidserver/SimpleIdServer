...
function install_scim_systemd {
    echo ""
    echo -e "${C_YELLOW}Install scim.service"
    echo -e "=======================${NC}"
    echo ""
    RUNDIR="$1"
    ...
    read -r -d '' SCIM_SYSTEMD_SERVICE_CONF << END
[Unit]
Description=SCIM
...
TimeoutStopSec=300
User=USER
ExecStart=SCIM_DIR/run.sh

[Install]
WantedBy=multi-user.target
END

    export SCIM_SYSTEMD_SERVICE_CONF
    export SYSTEMD_SERVICE_FILE=/etc/systemd/system/scim.service
    sudo -E bash -c 'echo "$SCIM_SYSTEMD_SERVICE_CONF" > "$SYSTEMD_SERVICE_FILE"'
    sudo sed -i "s/USER/$EXEC_USER/g" "$SYSTEMD_SERVICE_FILE"
    sudo sed -i "s/SCIM_DIR/$ESC_RUNDIR/g" "$SYSTEMD_SERVICE_FILE"

    sudo systemctl daemon-reload
    sudo systemctl enable scim.service
    echo " "
    echo "Starting service..."
    sudo systemctl restart scim.service
    echo " "
    sleep 5
    sudo systemctl status scim.service
    echo " "
    echo "For details try journalctl -u scim.service"
    echo ""

}

echo -e "${C_L_GREEN}${T_BOLD}Install SCIM /Service${NC}${NT}"
echo -e "${C_D_GRAY}==============================${NC}"
echo ""
