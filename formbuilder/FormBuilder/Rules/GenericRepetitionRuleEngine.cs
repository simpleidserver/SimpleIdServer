...
public abstract class GenericRepetitionRuleEngine<T> : IRepetitionRuleEngine where T : IRepetitionRule
{
    ...
    public List<(IFormElementRecord, JsonNode)> Transform(string fieldType, JsonObject input, IRepetitionRule parameter, Dictionary<string, object> parameters) => InternalTransform(fieldType, input, (T)parameter, parameters);
    ...
    protected abstract List<(IFormElementRecord, JsonNode)> InternalTransform(string fieldType, JsonObject input, T parameter, Dictionary<string, object> parameters);
