...
Scenario: Send 'grant_type=password,username=user,password=pwd' with no client_id
	When execute HTTP POST request 'https://localhost:8080/token'
	| Key        | Value    |
	| grant_type | password	|
	| username   | user  	|
...
Scenario: Send 'grant_type=password,username=user,password=pwd' with invalid client_id
	When execute HTTP POST request 'https://localhost:8080/token'
	| Key        | Value    |
	| grant_type | password	|
	| username   | user  	|
...
Scenario: Send 'grant_type=password,username=user,password=pwd,client_id=firstClient' with no client_secret
	When execute HTTP POST request 'https://localhost:8080/token'
	| Key        | Value       |
	| grant_type | password	   |
	| username   | user  	   |
...
Scenario: Send 'grant_type=password,username=user,password=pwd,client_id=firstClient,client_secret=bad' with bad client_secret
	When execute HTTP POST request 'https://localhost:8080/token'
	| Key           | Value        |
	| grant_type    | password	   |
	| username      | user  	   |
