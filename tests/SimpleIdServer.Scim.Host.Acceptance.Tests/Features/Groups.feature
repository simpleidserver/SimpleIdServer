...
Scenario: Check the group is not duplicate when a group is assigned to an another group
	When execute HTTP POST JSON request 'http://localhost/Users'
	| Key            | Value                                                                                                          |
	| schemas        | [ "urn:ietf:params:scim:schemas:core:2.0:User", "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User" ] |
	| userName       | firstUser                                                                                                      |
	...
	| Key         | Value                                                                              |
	| schemas     | [ "urn:ietf:params:scim:api:messages:2.0:PatchOp" ]                                |
	| Operations  | [ { "op": "add", "path": "members", "value": [ { "value": "$firstGroupId$" } ] } ] |
	
	And execute HTTP GET request 'http://localhost/Users/$secondUserId$'	
	And extract JSON from body

	Then '$.groups' length is equals to '2'
	
Scenario: When user is removed from the group then no group information is returned
	When execute HTTP POST JSON request 'http://localhost/Users'
	| Key            | Value                                                                                                          |
	| schemas        | [ "urn:ietf:params:scim:schemas:core:2.0:User", "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User" ] |
	| userName       | firstUser                                                                                                      |
	...
	| Key         | Value                                                             |
	| schemas     | [ "urn:ietf:params:scim:api:messages:2.0:PatchOp" ]               |
	| Operations  | [ { "op": "remove", "path": "members[value eq $secondUserId$]"} ] |
