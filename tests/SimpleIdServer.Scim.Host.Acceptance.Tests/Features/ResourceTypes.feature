...
Scenario: Check informations returned by ResourceTypes endpoint
	...
	Then JSON '[1].id'='Groups'
	Then JSON '[1].name'='Group'
	Then JSON '[1].description'='Group'
	Then JSON '[1].endpoint'='/Groups'
	Then JSON '[1].schema'='urn:ietf:params:scim:schemas:core:2.0:Group'
	Then JSON '[1].meta.resourceType'='Groups'