// Copyright (c) SimpleIdServer. All rights reserved.
// Licensed under the Apache License, Version 2.0. See LICENSE in the project root for license information.

using NUnit.Framework;
using SimpleIdServer.Did;
using SimpleIdServer.Did.Crypto.Multicodec;
using SimpleIdServer.Did.Encoders;
using SimpleIdServer.Did.Key;

namespace SimpleIdServer.DID.Tests;

public class JwtVerifierFixture
{
    [Test]
    public async Task When_Verify_ESBI_IdToken_Then_The_Signature_Is_Correct()
    {
        // ARRANGE
        const string idToken = "..LWcSQzPvuz6GsHKbcNDM3mzP-1BnFhwO9tqzltQ7l5AdmvJPOLKYDlkJ6-yivCcfkUZ-zrw38xLnAL2q1lkNNw";
        var resolver = new DidFactoryResolver(new List<IDidResolver>
        {
            DidKeyResolver.New(new DidKeyOptions
            {
                PublicKeyFormat = JsonWebKey2020Standard.TYPE
            })
        });
        var verifier = new JwtVerifier(resolver);

        // ACT
        var result = await verifier.VerifyWithJsonWebKey(idToken, CancellationToken.None);

        // ASSERT
        Assert.True(result.IsValid);
    }
}
