...
Scenario: auth_req_id has already been notified
	When add JSON web key to Authorization Server and store into 'jwks'
	| Type | Kid | AlgName |
	| SIG  | 1   | RS256   |
	...
	And notifiy authorization request '$auth_req_id$'
	...
	Then HTTP status code equals to '400'
	Then JSON 'error'='invalid_request'
	Then JSON 'error_description'='the authentication request '$auth_req_id$' has already been notified to the client'