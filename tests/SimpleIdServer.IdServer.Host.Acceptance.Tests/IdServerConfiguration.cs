...
public class IdServerConfiguration
{
    ...
    public static List<Client> Clients = new List<Client>
    {
        ...
        ClientBuilder.BuildTraditionalWebsiteClient("fiftyEightClient", "password", null, "http://localhost:8080").UseClientSecretPostAuthentication().AddAuthDataTypes("openid_credential").EnableTokenInResponseType().DisableConsent().AddScope(StandardScopes.GrantManagementQuery, StandardScopes.GrantManagementRevoke).Build(),
        ...
        ClientBuilder.BuildCredentialIssuerClient("seventyOneClient", "password").IsTransactionCodeRequired().AddScope(UniversityCredential).Build(),
        ClientBuilder.BuildWalletClient("seventyTwoClient", "password").Build()
    };
