...
Scenario: response_uri is required when the response_mode is equals to direct_post
	Given authenticate a user
	
	When execute HTTP GET request 'http://localhost/authorization'
	| Key                     | Value                                |
	| response_type           | code                                 |
	| client_id               | seventyFourClient                    |
	| state                   | state                                |
	...
	Then HTTP status code equals to '400'
	Then JSON 'error'='invalid_request'
	Then JSON 'error_description'='missing parameter response_uri'

Scenario: the parameters response_uri and request_uri cannot be used the same time
	Given authenticate a user
	...
	Then HTTP status code equals to '400'
	Then JSON 'error'='invalid_request'
	Then JSON 'error_description'='the response_uri cannot be used with redirect_uri'