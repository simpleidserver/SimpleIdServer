...
using IdServer;
using IdServer.Configurations;
using IdServer.Converters;
    ...
    using (var scope = app.Services.GetRequiredService<IServiceScopeFactory>().CreateScope())
    {
        using (var dbContext = scope.ServiceProvider.GetService<StoreDbContext>())
        {
            ...
            if (!dbContext.Realms.Any())
                dbContext.Realms.AddRange(IdServer.IdServerConfiguration.Realms);
            ...
            if (!dbContext.UmaPendingRequest.Any())
                dbContext.UmaPendingRequest.AddRange(IdServer.IdServerConfiguration.PendingRequests);

            if (!dbContext.UmaResources.Any())
                dbContext.UmaResources.AddRange(IdServer.IdServerConfiguration.Resources);

            if (!dbContext.GotifySessions.Any())
                dbContext.GotifySessions.AddRange(IdServer.IdServerConfiguration.Sessions);

            foreach (var language in IdServer.IdServerConfiguration.Languages)
                AddMissingLanguage(dbContext, language);

            foreach (var providerDefinition in IdServer.IdServerConfiguration.ProviderDefinitions)
            {
            ...
            if (!dbContext.IdentityProvisioningDefinitions.Any())
                dbContext.IdentityProvisioningDefinitions.AddRange(IdServer.IdServerConfiguration.IdentityProvisioningDefLst);

            if (!dbContext.IdentityProvisioningLst.Any())
                dbContext.IdentityProvisioningLst.AddRange(IdServer.IdServerConfiguration.GetIdentityProvisiongLst(scimBaseUrl));

            if (!dbContext.RegistrationWorkflows.Any())
                dbContext.RegistrationWorkflows.AddRange(IdServer.IdServerConfiguration.RegistrationWorkflows);
            ...
            if (!dbContext.CertificateAuthorities.Any())
                dbContext.CertificateAuthorities.AddRange(IdServer.IdServerConfiguration.CertificateAuthorities);

            if (!dbContext.PresentationDefinitions.Any())
                dbContext.PresentationDefinitions.AddRange(IdServer.IdServerConfiguration.PresentationDefinitions);

            if(!dbContext.FederationEntities.Any())
                dbContext.FederationEntities.AddRange(IdServer.IdServerConfiguration.FederationEntities);
        ...
        void AddMissingAuthenticationSchemeProviders(StoreDbContext dbContext)
        {
            ...
            {
                foreach (var provider in IdServer.IdServerConfiguration.Providers)
                {
        ...
        List<Scope> MigrateScopes(StoreDbContext dbContext, Realm masterRealm)
        {
            ...
            var unsupportedScopes = IdServer.IdServerConfiguration.Scopes.Where(s => !allScopeNames.Contains(s.Name));
        ...
        {
            var confClientIds = IdServer.IdServerConfiguration.Clients.Select(c => c.ClientId);
            var allClientIds = dbContext.Clients.Select(s => s.ClientId);
            var unknownClients = IdServer.IdServerConfiguration.Clients.Where(c => !allClientIds.Contains(c.ClientId));
            ...
            {
                var cl = IdServer.IdServerConfiguration.Clients.Single(c => c.ClientId == knownClient.ClientId);
